...
<div id="primary" data-post="<?php echo get_the_ID()?>" class="content-area-full riverjam">
  ...
  <?php
  $sponsor_visibility = get_field('sponsor_section_visibility');
  $sponsor_section_show = ($sponsor_visibility=='off') ? false : true;
  if($sponsor_section_show) {
    $sponsor_section_title = get_field('sponsor_section_title');
    $sponsors = get_field('sponsors');
    if($sponsors || $sponsor_section_title) { ?>
    <div class="flexibleContentWrap">
      <section class="sponsors-block" data-section="Sponsors" data-anchor-target="#river-jam-sponsors" style="background:#F2F2F2;padding:30px 20px 40px;">
        <?php if($sponsor_section_title) { ?>
        <div class="wrapper title-wrapper  no-section-text">
          <div class="titlediv">
            <div class="text-center">
              <h2 class="stitle"><?php echo $sponsor_section_title; ?></h2>
            </div>
          </div>
        </div>
        <?php } ?>
